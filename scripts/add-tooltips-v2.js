 ...
 * 1. More carefully parses React/TSX code
 * 2. Only modifies buttons that don't already have tooltips
 * 3. Properly inserts tooltip state management
...
function generateTooltipCode(tooltipId, tooltipText, buttonCode) {
  ...
  // Remove existing className if present and add flex-1
  let newAttrs = buttonAttrs.replace(/className="[^"]*"/g, '');
  if (!newAttrs.includes('className')) {
    newAttrs += ' className="flex-1 px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors duration-200 shadow-md"';
  } else {
...
function processFile(filePath) {
  ...
  // Find all router.push calls with routes
  ...
  // Find buttons that call router.push - use a more careful regex
  // Match button tags that contain router.push in their onClick
  const buttonRegex = /<button([^>]*onClick[^>]*)>([\s\S]*?)<\/button>/g;
  const buttons = [];
  let buttonMatch;
  
  while ((buttonMatch = buttonRegex.exec(content)) !== null) {
    const fullButton = buttonMatch[0];
    const onClickAttr = buttonMatch[1];
    
    // Check if this button has a router.push call
  ...
  buttons.reverse().forEach(({ fullButton, route }) => {
